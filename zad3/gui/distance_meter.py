...
receiver_signal_period_entry.bind("<FocusOut>", lambda e: freq_label.config(text="Frequency: " + str(1 / float(receiver_signal_period_entry.get())) + " Hz"))
...
def start_distance_meter():
    ...
    original_values, received_values, t_values, diff = distance_meter.calculate()
    for i in range(len(t_values)):
        data_table.insert("", "end", values=(t_values[i], original_values[i], received_values[i], diff[i]))

    print("Completed")
    distance_meter.show_plot()
