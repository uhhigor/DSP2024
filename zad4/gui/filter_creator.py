...
from zad4.api import signal_zad4
...
class filter_creator_frame:
    ...
    def execute_create_filter(self):
        ...
        if self.window_var.get() == "Okno Hamminga":
            window = signal_zad4.hamming_window
        if self.type_var.get() == "dolnoprzepustowy":
            self.t_values, self.y_values = signal_zad4.get_low_pass_filter(m, f0, fd, window)
        elif self.type_var.get() == "górnoprzepustowy":
            self.t_values, self.y_values = signal_zad4.get_high_pass_filter(m, f0, fd, window)
